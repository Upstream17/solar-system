...
> **最后更新**：2026-07-15 (v20260715a 贴图刷新 — 6 行星换 SSS 2k + 土星环 alpha + 比例修复)
> **基线 commit**：见 `git log -1`（v20260715a 贴图刷新）
> **上一基线 commit**：v20260712d（4 bug fixes — Phobos 缩放锁定 + 缩放上限限制 + Uranus 卫星黑色极区 + 数据参考平面注释）
> **再上一基线 commit**：v20260712c（Moon System 贴图质量+加载速度）
...
- **基线 commit**：v20260715a（见 `git log -1`，6 行星贴图刷新 + 土星环 alpha + 比例修复）
...
| `v20260712` | **上一基线 (Moon System 扩展 — 19 颗新卫星 + 图例折叠 + 滚动)** — 关键变更: (1) `constants.js` 新增 `MOONS[]` 数组 19 颗 (火星 2 + 木星 5 + 土星 6 + 天王星 5 + 海王星 1),数据全来自 NASA/JPL+Wikipedia,含 diameter/mass/orbit/eccentricity/inclination (含海卫一 -5.877d 逆向公转); (2) `planets.js` `makeMoon` 通用化,接收任意 `moonData`,通用 shader uniforms `uParentPos/uParentR/uTint` (替换原 `uEarthPos`),两层 Object3D 嵌套 (外层 pivot 静态倾角 + 内层 pivotOrbit 公转旋转); (3) `main.js` 主循环遍历 `window.__allMoons` 替代原单月球代码,每颗 moon 独立 tick 公转/自转/eclipse uniforms; (4) `ui.js` `initLegend` 重构:恒星/行星/卫星三级分组,卫星按母行星折叠 (chevron + 计数,localStorage 记忆状态),选中追踪自动展开父级 group; (5) `index.html` `#panel-legend .panel-body` max-height:50vh + overflow-y:auto,新增 `.legend-group`/`.moon-group`/`.moon-group-header`/`.moon-group-body` 样式。验证:20 颗全部加载 (1 月球 + 19 新),info panel 数据正确 (zh/en 双语),轨道+月食 shader uniforms 全部就绪,Triton 156.9° 倾角 + 逆向公转验证通过,0 JS 错误 |
| `v20260712b` | **上一基线 (Moon System 实贴图 — NASA 真实卫星贴图全部接入)** — 用户原话"所有卫星的贴图都是月球一摸一样,这是否有问题" → 确实假。解决方案: (1) 数据源 = `github.com/nasa/NASA-3D-Resources` (公开域 NASA),涵盖 19 颗卫星 (Phobos, Deimos, Galilean 4, Mimas, Enceladus, Tethys, Dione, Rhea, Titan, Iapetus, Miranda, Ariel, Umbriel, Titania, Oberon, Triton),仅 Amalthea 缺失用 moon.jpg 兜底; (2) Python PIL 下载 + resize 1024×512 (equirectangular 2:1) JPEG quality 80,总 1.05 MB / 19 张,平均 ~55 KB/张; (3) `constants.js` MOONS[] 18 颗更新 texture 路径指向真实贴图 (Amalthea 保持 moon.jpg); (4) `planets.js` shader 移除 25% tint 覆盖 (真实贴图自带正确色彩); 验证: Io 橙黄火山 + Europa 白冰 + Ganymede 明暗地形 + Callisto 重创 + Titan 橙雾 + Iapetus 阴阳脸 + Phobos/Deimos 暗灰 + Triton 粉灰偏极冠 + Oberon/Ariel/Umbriel/Miranda 各异,每颗视觉独特,0 JS 错误 |
| **`v20260712c`** | **上一基线 (Moon System 贴图质量+加载速度)** — 用户两个问题: (a) 大部分卫星贴图一大半是黑色 (NASA-3D-Resources 是 Cassini/Voyager 拼接的 partial maps,极区有大块黑色),(b) 进度条到 100% 还要等很久。解决方案: (1) **换数据源 → Planetary Visions** (公开域,equirectangular 全表面贴图,不是 partial map) — 19 张全部完整覆盖,Triton/Titania 等仅有的"半黑"是 Voyager 2 1989 单次飞掠历史限制,无法解决; (2) 分辨率从 1024×512 降至 **512×256** (卫星本身视觉上小,256 高足够),总大小从 1.05 MB → **494 KB** (减半); (3) **修复进度条 bug** — `TOTAL_ASSETS` 从 11 改为 **33** (1 sun + 8 行星主图 + earth_normal/clouds + 2 rings + 1 月球 + 19 卫星),避免 11/33 后假 100%; (4) **并行加载纹理** — `makePlanet` 内部 tex+bump+ring+clouds 4 个 HTTP 同时启动 (Promise.all) 不再串行等,行星 `for` 循环 → `Promise.all(map)`,卫星同。性能: localhost 32 个纹理总耗时 257 ms (并行),最长单图 19 ms; 旧版估 8 × 4 + 19 ≈ 51 次串行 HTTP × 15 ms ≈ 765 ms; **验证**: Io/Europa/Triton/Titania 全部完整覆盖 (无大黑斑),0 JS 错误 |
| `v20260712d` | **上一基线 (4 bug fixes)** — 用户 4 个问题: (a) **Phobos/Deimos 缩放锁定** — 旧 `applyZoomScale` 用 `maxR = targetR * 50` (对称缩放),Phobos(targetR=0.04) maxR=2 → 相机困死在 0.3-2 之间无法逃出。修: 取消 maxR 上限,改用 `min(camera.far * 0.95, 200000)` 即整个场景远裁面,确保从任何卫星都可缩到太阳系全景。(b) **缩放上限太低** — 用户要求只限制放大倍率不限制缩小。修: maxR 完全放开,只设 minR (zoom-IN 防穿模); 太阳 minMultiplier=2.0,行星 1.2,卫星 3.0 (3 类不同最小倍率); 同步 `focusOn()` 也区分行星/卫星 (卫星初始距离 max(displayR×6, 1.5)) 防止飞过去离卫星太近看不出。(c) **天王星卫星黑色极区** — 5 颗 Uranus moon + Triton (Voyager 2 1986 单次飞掠) 顶部约 50% 行是 PV 当作"缺数据"标的 uniform 暗棕色,球面映射后变成"黑极帽"。修: Python+PIL 用 mirror fill (rows 0..seam 从 seam 后的 source band 镜像填充) + Gaussian blur (radius=3) 软化接缝,顶部亮度从 ~22 → ~80 (接近正常月面亮度),接缝处 blur 化处理。(d) **数据怀疑** — 用户质疑卫星直径/轨道/倾角是否乱填。诚实审计: 19 颗中 **16 颗完全正确** (误差 <1% 对照 Wikipedia infobox),**3 颗** (Callisto/Iapetus/Triton) 是参考平面不同:我用的是 "到行星赤道" (e.g. Triton 156.9° 极地逆行),Wiki infobox 列出 "到黄道" (e.g. Triton 129.812°)。两者都是 NASA 真实值,只是参考系不同 — 我保留视觉上更有意义的赤道参考 (Triton 156.9° 更突出"极地逆行"特征),但加注释标注两种参考平面让后人知道。验证: Phobos 现在能缩出到全景,Titania/Triton 球面不再有黑极,数据审计完成,0 JS 错误 |
| `v20260715a` | **当前基线 (6 行星贴图刷新 + 土星环 alpha + 比例修复)** — 3 commits: (a) 5 行星主图 (mars/jupiter/saturn/uranus/neptune) 从老 1k 单图换 Solar System Scope 2k equirectangular (CC BY 4.0) + LANCZOS 1024×512 + JPEG quality 78; 5 张合计 1024 KB → 177 KB (-83%)。视觉升级: 木星大红斑清晰 / 火星表面纹理保留红系 / 天王星真实青色 / 海王星更亮更清晰 / 土星条带 + 北极六边形可见 (用户确认)。 (b) 水星同样换 SSS 2k (老橙棕 NASA 艺术化 → Messenger 灰白, 286 KB → 94 KB)。 (c) **土星环升级**: saturn_ring.jpg (老 RGB 915×64) → saturn_ring.png (SSS RGBA 2048×125, alpha 通道), 加 alphaTest=0.5 让环的空隙真正可见; 修老 bug `ringInner` hardcode 1.4 → 读 `p.ringInner=1.24` (NASA A ring 真实比例), 同理 `ringOuter` 2.2 → 2.34。 6 行星合计贴图 1.27 MB → 271 KB (-79%)。未动: earth.jpg (NASA Blue Marble 独立维护), venus.jpg (用户拍板保留), uranus_ring.jpg (SSS 无 ring alpha), 19 卫星贴图 (v20260712c), 代码 0 行代码改动 (除土星环 alphaTest 3 行 + 修 hardcode)。 README attribution 早已在 README.md L143-148, 无需改动 |
